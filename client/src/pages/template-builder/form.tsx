...
/**
 * Diff the template's current columns against the live target-table columns, matching by name.
 * Preserves CSV mappings for unchanged/changed columns; drops removed columns; appends added ones.
 */
function diffColumns(current: ColumnRow[], live: PersistenceTableColumn[]): ColumnDiffResult {
  ...
  const merged: ColumnRow[] = live.map((liveCol) => {
    const existing = currentByName.get(liveCol.name);
    if (!existing) {
      added.push(liveCol.name);
      return { ...liveCol, csvColumnName: null, csvColumnIndex: -1 };
    }
    if (
      existing.type !== liveCol.type ||
      existing.length !== liveCol.length ||
      existing.allowNull !== liveCol.allowNull
    ) {
      changed.push(liveCol.name);
    }
    return {
      ...liveCol,
      csvColumnName: existing.csvColumnName,
      csvColumnIndex: existing.csvColumnIndex,
    };
  });

  const removed = current.filter((c) => !liveNames.has(c.name)).map((c) => c.name);

  // Re-index by final display order, matching the positional re-index done on submit.
  const reindexed = merged.map((c, position) => ({ ...c, index: position }));

  return { merged: reindexed, added, removed, changed };
}
...
export function PersistenceTemplateFormDialog({
  ...
  const [syncPreview, setSyncPreview] = useState<ColumnDiffResult | null>(null);
  ...
  /** Diff the current columns against the live target-table schema and preview the merge. */
  const handleSyncClick = () => {
    ...
    if (!found) {
      toast({
        title: 'Warning',
        description: 'Target table metadata is not loaded yet.',
        variant: 'destructive',
      });
      return;
    }
    const diff = diffColumns(columns, found.columns);
    if (diff.added.length === 0 && diff.removed.length === 0 && diff.changed.length === 0) {
      toast({
        title: 'Columns already in sync',
        description: 'No differences found with the target table.',
      });
      return;
    }
    setSyncPreview(diff);
  ...
  /** User accepted the sync preview - apply the merged columns. */
  const handleSyncAccept = () => {
    if (syncPreview) {
      setColumns(syncPreview.merged);
    }
    setSyncPreview(null);
    setSyncDialogOpen(false);
  };

  /** User cancelled the sync preview. */
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} autoComplete="off">
          <div className="space-y-4 py-4">
            ...
            {/* Confirmation dialog - sync columns with target table */}
            <Dialog open={syncDialogOpen} onOpenChange={setSyncDialogOpen}>
              <DialogContent className="max-w-sm">
                <DialogHeader>
                  <DialogTitle>Sync Columns with Target Table</DialogTitle>
                  <DialogDescription>
                    The target table&apos;s columns have changed since this template was last
                    saved. Existing CSV mappings are preserved for unchanged columns.
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-2 text-sm">
                  {syncPreview && syncPreview.added.length > 0 && (
                    <div>
                      <p className="font-medium text-uds-system-green-700">Added</p>
                      <p className="text-muted-foreground">{syncPreview.added.join(', ')}</p>
                    </div>
                  )}
                  {syncPreview && syncPreview.removed.length > 0 && (
                    <div>
                      <p className="font-medium text-uds-system-red-700">Removed</p>
                      <p className="text-muted-foreground">{syncPreview.removed.join(', ')}</p>
                    </div>
                  )}
                  {syncPreview && syncPreview.changed.length > 0 && (
                    <div>
                      <p className="font-medium text-uds-system-amber-700">Changed</p>
                      <p className="text-muted-foreground">{syncPreview.changed.join(', ')}</p>
                    </div>
                  )}
                </div>
                <DialogFooter>
                  <Button variant="outline" onClick={handleSyncCancel}>
                    Cancel
                  </Button>
                  <Button onClick={handleSyncAccept}>Accept</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
