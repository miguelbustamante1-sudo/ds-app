...
import type { CompensatoryTimeDTO, CompStatus } from '@shared/dto/CompensatoryTime';
import {
  Toolbar,
  ToolbarActions,
  ToolbarDescription,
...
export function ApprovalManagementPage() {
  ...
  const [bulkConfirm, setBulkConfirm] = useState<{ status: CompStatus; label: string; count: number } | null>(null);
  ...
  const loadData = async (page: number, pageSize: number, statuses?: CompStatus[], reportLevels?: number[], searches?: Record<string, string>, sort?: { id: string; desc: boolean }) => {
    setLoading(true);
    try {
  ...
  const openBulkConfirm = (status: CompStatus, label: string) => {
  ...
  useEffect(() => {
    const statuses = Array.isArray(statusFilterValue) && (statusFilterValue as string[]).length > 0
      ? (statusFilterValue as CompStatus[])
      : undefined;
    ...
    <div className="container">
      <Toolbar>
        ...
        <ToolbarActions>
          ...
          const statuses = Array.isArray(statusFilter?.value) && statusFilter.value.length > 0
              ? (statusFilter.value as CompStatus[])
              : undefined;
      ...
      <div className="mt-6 flex items-center gap-3">
        ...
        {(() => {
          ...
          const allOptions: { status: CompStatus; label: string }[] = [
