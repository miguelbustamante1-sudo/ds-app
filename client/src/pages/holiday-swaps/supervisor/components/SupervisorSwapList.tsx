...
interface SupervisorSwapListProps {
...
export function SupervisorSwapList({
  swaps,
  loading,
  operationLoading,
  tentativeStatusId,
  acknowledgedStatusId,
  onEditClick,
  onCancelClick,
  onApprove,
  onReject,
}: SupervisorSwapListProps) {
  const columns = useMemo<ColumnDef<HolidaySwapDTO>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const isTentative = tentativeStatusId !== null && swap.statusId === tentativeStatusId;
          const canEdit = !NON_EDITABLE_STATUSES.includes(swap.statusName);
          const canCancel = !NON_CANCELLABLE_STATUSES.includes(swap.statusName);
    ...
    [tentativeStatusId, operationLoading, onEditClick, onCancelClick, onApprove, onReject]
  );
