...
/** Zero-pads a whole-hour value to 2 digits: 0 -> "00", 8 -> "08", 18 -> "18" */
function padHour(h: number): string {
  return String(h).padStart(2, '0');
}

// ---------------------------------------------------------------------------
// Local state shape for detail rows
// Internal endHour = startHour + hours (lunch NOT included).
// Lunch is added on display and on save.
// ---------------------------------------------------------------------------
...
interface DetailState {
  ...
  dayOfWeek:     number;
  startHour:     number;
  endHour:       number; // = startHour + hours (no lunch)
}
...
export function ShiftEditDialog({ shift, open, onOpenChange, onSuccess }: Props) {
  ...
  // Derived total: sum of (endHour - startHour) for each detail row
  ...
  // Populate both sections whenever the dialog opens or the shift changes
  useEffect(() => {
    ...
    // DB endTime = startTime + hours + lunch; strip lunch so internal state = startTime + hours
    setDetails(sorted.map((d) => {
      ...
      return {
        ...
        startHour:     d.startTime,
        endHour:       strippedEnd,
      };
    }));
  }, [open, shift, resetMaster]);

  // Keep totalWeekHours in sync with detail changes
  useEffect(() => {
    setValue('totalWeekHours', computedTotalHours);
  }, [computedTotalHours, setValue]);

  // ---- Update a single field in a detail row ----
  const setDetailField = (
    index: number,
    field: 'startHour' | 'hours',
    value: number,
  ) => {
    setDetails((prev) =>
      prev.map((row, i) => {
        ...
        // field === 'hours': derive endHour from startHour + hours
        return { ...row, endHour: row.startHour + Math.max(0, value) };
      }),
    );
  };

  // ---- Unified save (master + details) ----
        ...
        <form onSubmit={handleSubmit(onSave)}>
          ...
          <div className="grid grid-cols-2 gap-4 py-2">
            ...
            <div className="space-y-2">
              ...
              {/* Hidden input keeps the value in the form so it gets submitted */}
                ...
                {details.map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Start (h) - stepper with zero-padded display */}
                      ...
                      {/* End (h) - read-only, startHour + hours + lunchHours (if hours > 0) */}
