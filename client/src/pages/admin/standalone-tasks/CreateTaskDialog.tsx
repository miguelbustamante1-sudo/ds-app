...
export function CreateTaskDialog({ open, onOpenChange, onSuccess }: CreateTaskDialogProps) {
  ...
  const watchedHierarchyContextId = watch('hierarchyContextId');
  ...
  const onSubmit = async (data: CreateTaskFormData) => {
    try {
      const payload: CreateStandaloneTaskDTO = {
        ...
        hierarchyContextId: data.hierarchyContextId ? parseInt(data.hierarchyContextId, 10) : null,
      ...
      <DialogContent className="max-w-lg">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <ComboBox
              options={teamMemberOptions}
              value={watchedHierarchyContextId}
              onValueChange={(value) => setValue('hierarchyContextId', value)}
              placeholder="Select the team member this task concerns..."
            />
          </div>
