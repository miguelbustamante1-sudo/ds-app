...
export function RoutePanel({ wflId, routes, tasks, onRefresh, isDraft }: RoutePanelProps) {
  ...
  const getOutcomeLabel = (wflId: string, wtoId: string | null): string => {
    if (!wtoId) return '-';
  ...
  const onSubmit = async (data: RouteFormData) => {
    ...
    const payload = {
      ...
      conditionType,
      routeOrder: parseInt(data.routeOrder, 10) || 0,
    };
  ...
  const columns = useMemo<ColumnDef<WtrWorkflowTemplateRoute>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => getOutcomeLabel(wflId, row.original.wtoId),
        size: 160,
        ...
        <SheetContent className="w-[480px] sm:max-w-[480px] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-4">
            ...
            <div className="space-y-1.5">
              ...
              <Input id="route-order" type="number" {...register('routeOrder')} placeholder="0" />
            </div>
