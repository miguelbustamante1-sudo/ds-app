...
export function validateDaysBefore(
  ...
  return {
    valid: false,
    errorMessage: `Policy requires that time-off requests for "${categoryName}" be submitted at least ${categoryCountryDaysBefore} days before the start date. The earliest valid start date from today is ${formattedDate}.`,
    earliestValidDate,
    daysUntilStart,
  };
}
