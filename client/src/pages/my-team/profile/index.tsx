...
// Split (sta_id = 6) is excluded unconditionally, matching every supervisor query in
// src/services/timeoff/supervisor/queries.ts (`sta_id NOT IN (4, 6)`) — never shown here
// regardless of the Show past / Show cancelled checkboxes below.
...
export function TeamMemberProfilePage() {
  ...
  const filteredTimeOffs = useMemo(() => {
    ...
    return timeOffs.filter((t) => {
      if (t.statusId === SPLIT_STATUS_ID) return false;
