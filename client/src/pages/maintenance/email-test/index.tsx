...
export function EmailTestPage() {
  ...
  const runPreset = async (index: number) => {
    ...
    try {
      ...
      await apiPost<{ sent: boolean }>('/api/email-test/send', {
        to,
        subject: fullSubject,
        body,
      ...
      setResults((prev) =>
        prev.map((r, i) => (i === index ? { ...r, status: 'ok' } : r)),
      );
    } catch (err: unknown) {
  ...
  const statusBadge = (r: SendResult) => {
    ...
    if (r.status === 'ok') return <Badge variant="default" className="bg-green-600">Sent</Badge>;
