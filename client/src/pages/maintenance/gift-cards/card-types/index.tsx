...
export function GiftCardTypesPage() {
  const [cardTypes, setCardTypes] = useState<GiftCardTypeDTO[]>([]);
  const [loading, setLoading] = useState(false);
  const [sorting, setSorting] = useState<SortingState>([]);
  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);
  const [formOpen, setFormOpen] = useState(false);
  const [selectedCardType, setSelectedCardType] = useState<GiftCardTypeDTO | null>(null);
  const [deactivateTarget, setDeactivateTarget] = useState<GiftCardTypeDTO | null>(null);
  ...
  const loadCardTypes = async () => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: err instanceof Error ? err.message : 'Failed to load card types',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  useEffect(() => { loadCardTypes(); }, []);

  const handleEdit = (cardType: GiftCardTypeDTO) => {
  ...
  const handleDeactivateConfirm = async () => {
    ...
    try {
      await deactivateCardType(deactivateTarget.cardTypeId);
      toast({ title: 'Deactivated', description: `Card type "${deactivateTarget.cardTypeName}" deactivated.` });
      setDeactivateTarget(null);
      loadCardTypes();
    } catch (err: unknown) {
      toast({
        title: 'Error',
        description: err instanceof Error ? err.message : 'Failed to deactivate card type',
        variant: 'destructive',
      });
    } finally {
  ...
  const columns = useMemo<ColumnDef<GiftCardTypeDTO>[]>(
    () => [
      ...
      {
        accessorKey: 'cardTypeIsActive',
        header: ({ column }) => <DataGridColumnHeader column={column} title="Status" />,
        cell: ({ row }) =>
          row.original.cardTypeIsActive ? (
            <Badge variant="success">Active</Badge>
          ) : (
            <Badge variant="secondary" className="opacity-60">Inactive</Badge>
          ),
        filterFn: (row, _, filterValues: string[]) =>
          filterValues.includes(String(row.original.cardTypeIsActive)),
        size: 100,
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-1">
            ...
            {canDelete('GiftCardCatalog') && row.original.cardTypeIsActive && (
              <Button
                variant="ghost"
                size="sm"
  ...
  return (
    <div className="container">
      <Toolbar>
        <ToolbarHeading>
          <ToolbarPageTitle>Gift Card Types</ToolbarPageTitle>
          <ToolbarDescription>Manage gift card providers and brands</ToolbarDescription>
        </ToolbarHeading>
        <ToolbarActions>
          <Button variant="outline" onClick={loadCardTypes} disabled={loading}>
            <RefreshCw size={16} className={`me-1 ${loading ? 'animate-spin' : ''}`} />
            Refresh
          </Button>
          {canCreate('GiftCardCatalog') && (
            <Button onClick={handleAdd}>+ Add Card Type</Button>
          )}
        </ToolbarActions>
      </Toolbar>
      ...
      <div className="flex items-center gap-2 mt-6">
        <DataGridColumnFilter
          column={table.getColumn('cardTypeIsActive')}
          title="Status"
      ...
      <AlertDialog open={!!deactivateTarget} onOpenChange={(open) => { if (!open) setDeactivateTarget(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Deactivate Card Type</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to deactivate <strong>{deactivateTarget?.cardTypeName}</strong>?
