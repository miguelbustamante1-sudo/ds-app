...
export function RequestFormDialog({ open, onOpenChange, onSuccess }: Props) {
  ...
  const {
    register, handleSubmit, reset, control, watch, setValue, setError, clearErrors,
    formState: { errors },
  ...
  const onSave = async (data: FormData) => {
    const cleanRecipients = data.recipients.map(r => r.email.trim()).filter(r => r !== '');

    if (data.assignmentAmount > 1 && cleanRecipients.length !== data.assignmentAmount) {
      setError('recipients', {
        type: 'manual',
        message: `${data.assignmentAmount} recipients are required for ${data.assignmentAmount} cards. Currently there are ${cleanRecipients.length}.`,
      });
      return;
    }
    if (cleanRecipients.length < 1) {
      setError('recipients', { type: 'manual', message: 'At least one recipient is required' });
      return;
    }
    ...
    try {
      ...
      toast({ title: `Request #${result.assignmentId} submitted`, description: 'The gift card distribution request was created successfully. Status: Pending authorization.' });
      onSuccess();
      onOpenChange(false);
    } catch (err: unknown) {
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label>
                ...
                {amount > 1 && <span className="text-muted-foreground"> ({recipients.filter(r => r.email.trim()).length}/{amount})</span>}
              </Label>
              <Button type="button" variant="outline" size="sm" onClick={() => appendRecipient({ email: '' })}>+ Add email</Button>
            </div>
            <div className="space-y-2">
              {recipientFields.map((field, idx) => (
                <div key={field.id} className="flex items-center gap-2">
                  ...
                  {recipientFields.length > 1 && (
                    <Button type="button" variant="ghost" size="sm" onClick={() => removeRecipient(idx)}>
                      <X size={16} />
                    </Button>
                  )}
                </div>
