...
export function KnowledgeBasePage() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast({
        title: 'Uploaded',
        description: `${file.name} ingested.`,
      });
      void loadDocs();
    } catch (err: unknown) {
      toast({
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await apiDelete(`/api/sop/documents/${encodeURIComponent(deletingDoc.fileId)}`);
      toast({ title: 'Deleted', description: `${deletingDoc.filename} removed.` });
      setDocs((prev) => prev.filter((d) => d.fileId !== deletingDoc.fileId));
    } catch {
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove &ldquo;{deletingDoc?.filename}&rdquo; from the
