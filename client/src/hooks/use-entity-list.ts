...
export interface UseEntityListOptions<TEntity> {
  endpoint: string;
  idKey: keyof TEntity;
...
export interface UseEntityListReturn<TEntity, TCreateDTO, TUpdateDTO> {
  ...
  updateItem: (id: number | string, data: TUpdateDTO) => Promise<TEntity>;
...
export function useEntityList<TEntity, TCreateDTO, TUpdateDTO>({
  ...
  onSuccess,
  onError,
}: UseEntityListOptions<TEntity>): UseEntityListReturn<TEntity, TCreateDTO, TUpdateDTO> {
  ...
  const updateItem = useCallback(async (id: number | string, data: TUpdateDTO): Promise<TEntity> => {
    try {
      const updated = await apiPut<TEntity, TUpdateDTO>(`${endpoint}/${id}`, data);
      setItems(prev => prev.map(item => (item[idKey] as unknown) === id ? updated : item));
  ...
  const deleteItem = useCallback(async (id: number | string): Promise<void> => {
    try {
      await apiDelete(`${endpoint}/${id}`);
      setItems(prev => prev.filter(item => (item[idKey] as unknown) !== id));
