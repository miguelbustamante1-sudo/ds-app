  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      res.cookie(
        "access_token",
        tokens.access_token,
        getAccessTokenCookieOptions(expiresIn),
      );
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to exchange code" });
    }
  },
);
...
router.get("/login", (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ error: error.message || "Failed to initiate login" });
  }
});
...
router.get("/callback", async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ error: error.message || "Failed to handle callback" });
  }
});
...
router.post("/refresh", async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // ── DEV FAST-PATH ────────────────────────────────────────────────────────
    // In local development, OneLogin is not configured. When the dev-login
    // endpoint creates a session it signs the refresh_token with JWT_SECRET
    // (HS256). Detect that here and renew it locally — no OneLogin calls needed.
    //
    // FIX NOTE: If a future pull from main breaks dev login again (401 on
    // /api/auth/refresh), check whether this block was overwritten. The symptom
    // is "Error: Missing OneLogin configuration" in container logs.
    if (process.env.NODE_ENV !== "production") {
      try {
        const JWT_SECRET =
          process.env.JWT_SECRET || "dev-secret-key-change-in-production";
        const decoded = jwt.verify(refreshToken, JWT_SECRET) as Record<string, any> & { email?: string };

        if (decoded?.email) {
          const expiresIn = 86400; // 24 hours — same as dev-login
          const expiresAt = Date.now() + expiresIn * 1000;

          // Re-issue the token with fresh expiry
          ...
          // Resolve user + permissions from DB (same as prod path, no OneLogin)
          // Cast is safe: dev-login creates a token with all TokenPayload required fields
          const permissionsSource =
            (process.env.PERMISSIONS_SOURCE as PermissionSource | undefined) || "db";
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const devPayload = decoded as any;
          const user = await userService.syncUserFromToken(devPayload);
          const permissions = await resolvePermissions(permissionsSource, {
            userId: user.id,
            tokenPayload: devPayload,
          });
        ...
        // Not a local dev JWT — fall through to the OneLogin path below
      }
    }
    // ── END DEV FAST-PATH ────────────────────────────────────────────────────
...
router.get("/dev-config", (_req: AuthenticatedRequest, res: Response) => {
  const enableDevLogin =
    ...
    (process.env.ENABLE_DEV_LOGIN === "true" ||
      process.env.VITE_ENABLE_DEV_LOGIN === "true");

  res.json({
    enableDevLogin,
    devUsername: enableDevLogin
      ? process.env.DEV_USERNAME || process.env.VITE_DEV_USERNAME || ""
      : "",
  });
});
...
router.post("/dev-login", async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const DEV_USER_PASSWORD =
      process.env.DEV_USER_PASSWORD || "dev-password-123";
    ...
    if (!user) {
      res
        ...
        .json({
          error:
            "Test user not found in database. Please ensure the user exists in sec.auth_users",
        });
      return;
    }
...
/**
 * Sub-router: /api/auth/me/preferences
 * GET  — fetch current user's UI preferences
 * PATCH — update current user's UI preferences
 */
