...
router.post('/split', requirePermission('TimeOffs', 'create'), resolveAuthUser, async (req, res: Response) => {
  try {
    ...
    const defaultLogComment = comment?.trim() || 'SV vacation split request created';
...
router.post('/:timeOffId/convert-to-split', requirePermission('TimeOffs', 'create'), resolveAuthUser, async (req, res: Response) => {
  try {
    ...
    // Atomic transaction: mark original as split origin, create two children
    ...
    const defaultLogComment = comment?.trim() || 'SV vacation converted to split from edit page';
    const logCommentA = legAIsException
      ? 'Split Period A saved pending exception authorization (insufficient notice)'
      : defaultLogComment;
    const logCommentB = legBIsException
    ...
    await Promise.all([
      ...
      auditOrchestrator.log({
        ...
        newValues: createdA as unknown as Record<string, unknown>,
        comment: legAIsException
          ? `Split Period A created pending exception authorization from edit — linked to origin ${timeOffId}`
          : `Split Period A created from edit — linked to origin ${timeOffId}`,
      }),
      auditOrchestrator.log({
        entityName: 'tbl_tms_time_off',
        entityId: String(createdB.timeOffId),
        ...
        newValues: createdB as unknown as Record<string, unknown>,
        comment: legBIsException
          ? `Split Period B created pending exception authorization from edit — linked to origin ${timeOffId}`
          : `Split Period B created from edit — linked to origin ${timeOffId}`,
      }),
    ]);
...
router.post('/', requirePermission('TimeOffs', 'create'), resolveAuthUser, async (req, res: Response) => {
  try {
    const authReq = req as ResolvedAuthRequest;
    const { teamMemberId, resolvedUserId: userId } = authReq;
    ...
    if (!validationResult.valid) {
      ...
      if (onlyDaysBeforeFailure) {
        ...
        const { created } = await startExceptionAuthorization({
          teamMemberId,
          timeOffStartDate,
          timeOffEndDate,
          categoryId,
