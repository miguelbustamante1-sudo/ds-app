...
export async function loadGuatemalaPersonalDaysContext(
  ...
  const [personalDaysCancelledStatus, personalDaysRejectedStatus] = await Promise.all([
  ...
  const personalDaysExcludedStatusIds = [
    SPLIT_STATUS_ID,
    ...(personalDaysCancelledStatus ? [personalDaysCancelledStatus.statusId] : []),
    ...(personalDaysRejectedStatus ? [personalDaysRejectedStatus.statusId] : []),
  ];

  const andConditions: object[] = [
    ...(personalDaysExcludedStatusIds.length > 0
      ? [{ NOT: { statusId: { in: personalDaysExcludedStatusIds } } }]
      : []),
