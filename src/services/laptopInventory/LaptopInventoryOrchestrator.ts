      ...
      const laptop = await insertLaptop(
        {
          ...
          osVersion:    dto.osVersion?.trim() ?? null,
          blueprintName: dto.blueprintName?.trim() ?? null,
          tags:         dto.tags ?? [],
          lastCheckIn:  parseDateTime(dto.lastCheckIn),
          createdBy,
          createdDate: now,
        },
        tx,
    ...
    const { updatedLaptop } = await prisma.$transaction(async (tx) => {
      const updatedLaptop = await patchLaptop(
        laptopId,
        {
          ...
          ...(dto.deviceName     !== undefined && { deviceName:     dto.deviceName?.trim()     ?? null }),
          ...(dto.osVersion      !== undefined && { osVersion:      dto.osVersion?.trim()      ?? null }),
          ...(dto.blueprintName  !== undefined && { blueprintName:  dto.blueprintName?.trim()  ?? null }),
          ...(dto.tags           !== undefined && { tags:           dto.tags }),
          ...(dto.lastCheckIn    !== undefined && { lastCheckIn:    parseDateTime(dto.lastCheckIn) }),
          updatedBy,
          updatedDate: now,
        },
        tx,
