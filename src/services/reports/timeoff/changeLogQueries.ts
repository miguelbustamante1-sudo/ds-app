...
function buildJoins(): string {
  return `
    ...
    LEFT  JOIN ds.tot_time_off_types    oc  ON oc.tot_id   = (toc.toc_old_values->>'tot_id')::int
    LEFT  JOIN ds.tbl_to_statuses      os  ON os.sta_id   = (toc.toc_old_values->>'sta_id')::int
    -- Resolve new category / status names from JSON
    LEFT  JOIN ds.tot_time_off_types    nc  ON nc.tot_id   = (toc.toc_new_values->>'tot_id')::int
    LEFT  JOIN ds.tbl_to_statuses      ns  ON ns.sta_id   = (toc.toc_new_values->>'sta_id')::int
  `;
}
...
export async function getTimeOffChangeLog(
  ...
  const selectCols = `
    ...
    (toc.toc_old_values->>'tto_stadat')                        AS orig_start_date,
    (toc.toc_old_values->>'tto_enddat')                        AS orig_end_date,
    ...
    END                                                      AS orig_active,
    -- New values
    (toc.toc_new_values->>'tto_stadat')                        AS new_start_date,
    (toc.toc_new_values->>'tto_enddat')                        AS new_end_date,
