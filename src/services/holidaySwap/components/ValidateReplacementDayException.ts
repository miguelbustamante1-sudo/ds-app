...
export interface ReplacementDayExceptionInput {
  teamMemberId: number;
  countryId: number;
  proposedDate: Date;
...
// and the same-day-as-original check (REPLACEMENT_SAME_AS_ORIGINAL), allowing BSA exception
// operations with any replacement date — including past dates before or on the original holiday.
export async function validateReplacementDayException(
  ...
  // Steps 1 (REPLACEMENT_NOT_FUTURE) and 2 (REPLACEMENT_SAME_AS_ORIGINAL) are intentionally
  ...
  // 4. Not already a replacement date for another active swap of the same TM
  ...
  // 5. Not within an existing active TimeOff request
