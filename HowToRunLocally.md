...
That's it — Node.js is **not** required on your machine. Everything runs inside Docker.
...
| `DEV_USERNAME`         | **Set this** — email of the user you want to log in as (must exist in DB)   | *(set your email)*       |
| `VITE_DEV_USERNAME`    | **Set this** — same email, pre-fills the login form                         | *(set your email)*       |
| `VITE_ENABLE_DEV_LOGIN`| Enables the dev login button on the sign-in page                           | `true`                   |

> **Note:** The `DATABASE_URL`, `DB_HOST`, and `DB_PORT` values in `.env.local` are for reference only. When running in Docker, the `docker-compose.local.yml` overrides these to use the internal Docker network (`postgres-local:5432`).
...
### Important: Update the seed data

Before running the script, open `scripts/seed.sql` and update the email in steps 8 and 9 to match the `DEV_USERNAME` you set in `.env.local`. This is the email used for dev login.
...
> **Note:** The email you use must exist in both `sec.auth_users` and `ds.tbl_team_members`. The "Sign in with OneLogin" button won't work locally since it requires OneLogin SSO configuration.
...
- **Dev login says "Test user not found":** You need to insert a user row in the database with the email matching `DEV_USERNAME`.
- **Code changes not reflected:** The app runs a compiled build inside Docker. After code changes, rebuild with `docker compose -f docker-compose.local.yml up -d --build app`.
