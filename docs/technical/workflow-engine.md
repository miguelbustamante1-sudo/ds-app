...
**No real domain has adopted this yet.** Everything above is fully built and enforced, but — as of this writing — no template in any environment is configured as `DATABASE`-type, and no domain has written the stored procedures this path calls. Time Off's exception-authorization flow (this section, and the [domain integration guide](./workflow-domain-integration-guide.md)) remains entirely `CODE`-type.
...
5. **`DATABASE` execution type has no real adopter yet** ([§11a](#11a-domain-side-effects--database-execution-type)) — the stored-procedure outcome/instantiation path is fully built and validated at publish time, but no domain has written an instantiate/outcome procedure or configured a template as `DATABASE`-type. The optional audit-snapshot fields on `invokeDatabaseOutcomeProcedure`'s return value are similarly inert until a domain populates them. The admin UI has no fields yet to set `executionType`/proc names on a template or outcome — that configuration is DB-only today.
