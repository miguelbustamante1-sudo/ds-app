...
| Plan | Scope | Status |
|---|---|---|
| [PLAN-01](./PLAN-01-timeoff-diff-ui.md) | Shared diff-computation utilities + `ChangeLogDiff` component + wire into all 3 time-off detail pages. Frontend only. | `[ ] PENDING` |
| [PLAN-02](./PLAN-02-swap-history-tm-supervisor.md) | Audit read path (`AuditOrchestrator.getHistory`), `GET /api/holiday-swaps/:id/history`, wire `ChangeLogDiff` into the existing shared TM/supervisor swap detail page. | `[ ] PENDING` |
